...
//            MQLog("responseObject=\(responseObject),isBadNet=\(isBadNet)")
    ...
    func getRoomGroupListData(_ finishCallBack:@escaping ()->()) {
        ...
        MQNetworkingTool.sendPostRequest(hotUrl, succeed: { [unowned self] (responseObject, isBadNet) in
//            MQLog("responseObject=\(responseObject),isBadNet=\(isBadNet)")
        ...
        MQNetworkingTool.sendGetRequest(perttyUrl, parameters: perttyDict, succeed: { [unowned self] (responseObject, isBadNet) in
//            MQLog("responseObject=\(responseObject),isBadNet=\(isBadNet)")
